...
export function useProtocolites() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SpreaderNFT[]> => {
      ...
      const loadSpreader = async (tokenId: number): Promise<SpreaderNFT | null> => {
        try {
    ...
    enabled: !!publicClient && !!totalSupply && totalSupply > 0n,
...
async function loadInfections(
  ...
  try {
    ...
    const infectionIds = Array.from({ length: infectionCount }, (_, i) => i + 1)

    // Batch size for infections: process 20 at a time (they're smaller)
    const INFECTION_BATCH_SIZE = 20

    const loadInfection = async (tokenId: number): Promise<InfectionNFT | null> => {
      try {
    ...
    const results = await processBatch(infectionIds, INFECTION_BATCH_SIZE, loadInfection)
