...
export default function Home() {
  ...
  const {
    ...
    queryFn: (db) => db.select({ count: count(spreader.id) }).from(spreader),
  });
  ...
  const {
    ...
    queryFn: (db) => db.select({ count: count(infection.id) }).from(infection),
  });

  // Query spreaders with filtering and sorting
  const { data: spreaders = [], isLoading: isLoadingSpreaders } =
    usePonderQuery({
      queryFn: (db) => {
        // Build complete query without reassignments to avoid TypeScript issues
        const baseQuery = db.select().from(spreader);

        // Build query based on filter and sort combinations
        if (filter === "with-infections") {
          const filteredQuery = baseQuery.where(gt(spreader.infectionCount, 0));

          if (sort === "id-asc") {
            return filteredQuery.orderBy(asc(spreader.tokenId));
          } else if (sort === "id-desc") {
            return filteredQuery.orderBy(desc(spreader.tokenId));
          } else if (sort === "infections-desc") {
            return filteredQuery.orderBy(desc(spreader.infectionCount));
          } else if (sort === "infections-asc") {
            return filteredQuery.orderBy(asc(spreader.infectionCount));
          }
          return filteredQuery;
        } else {
          // No filter, just sort
          if (sort === "id-asc") {
            return baseQuery.orderBy(asc(spreader.tokenId));
          } else if (sort === "id-desc") {
            return baseQuery.orderBy(desc(spreader.tokenId));
          } else if (sort === "infections-desc") {
            return baseQuery.orderBy(desc(spreader.infectionCount));
          } else if (sort === "infections-asc") {
            return baseQuery.orderBy(asc(spreader.infectionCount));
          }
          return baseQuery;
        }
      },
  ...
  const { data: infections = [], isLoading: isLoadingInfectionsData } =
    usePonderQuery({
